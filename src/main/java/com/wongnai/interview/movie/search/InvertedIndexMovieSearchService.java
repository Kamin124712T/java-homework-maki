...
public class InvertedIndexMovieSearchService implements MovieSearchService {
    ...
    public List<Movie> search(String queryText) {
		...
		for (int i = 0; i < s.length; i++) {
			if(!InvertedIndex.iIdex.containsKey(s[i].toLowerCase())){
				return new ArrayList<>();
			}
			List<Long> id = InvertedIndex.iIdex.get(s[i].toLowerCase());
			if(movieIdList.size() == 0){
				movieIdList.addAll(id);
			}else {
				List<Long> intersect = movieIdList.stream()
						.filter(id::contains)
						.collect(Collectors.toList());
				movieIdList = intersect;
			}
		}
		return (List<Movie>) movieRepository.findAllById(movieIdList);
    }
}
