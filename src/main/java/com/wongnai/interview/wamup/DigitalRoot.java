...
public class DigitalRoot {
    public Object check(long number) {
        ...
        if (number < 0){
            throw new InputMismatchException("input is negative integer");
        }
        long rn = number;
        do {
            long n = rootNumber(rn);
            rn = n;
        } while (String.valueOf(rn).length() != 1);
        return rn;
    }

    private long rootNumber(long number) {
        int n = 0;
        String s = String.valueOf(number);
        for (int i = 0; i < s.length(); i++) {
            int ni = s.charAt(i);
            if (Character.isDigit(ni)) {
                n += Character.getNumericValue(ni);
            }
        }
        return n;
    }
